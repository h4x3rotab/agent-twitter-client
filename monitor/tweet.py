...
HOURS_THRESHOLD = int(os.getenv("HOURS_THRESHOLD", "3"))
...
def check_last_tweet():
    ...
    if "timestamp" not in data:
        raise KeyError(f"Error: File '{LAST_TWEET_FILE_PATH}' does not contain a 'timestamp' key.")
    ...
    try:
        file_timestamp = datetime.fromtimestamp(data["timestamp"] / 1000)  # Convert from milliseconds to seconds
    ...
    if file_timestamp < threshold_time:
        raise ValueError(f"Error: The timestamp in '{LAST_TWEET_FILE_PATH}' is older than {HOURS_THRESHOLD} hours.")
